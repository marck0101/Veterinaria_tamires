...
if (!$post) {
    header('Location: /admin/posts.php?msg=nao-encontrado');
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['remover_capa']) && $_POST['remover_capa'] === '1') $capa = '';

    if (!$erro && $titulo && $conteudo) {
        try {
            // Slug único (ignora o próprio post)
            $slug = slugUnico($pdo, $titulo, $id);

            $pdo->prepare(
                "UPDATE posts SET titulo=?,slug=?,resumo=?,conteudo=?,categoria=?,imagem=?,publicado=? WHERE id=?"
            )->execute([$titulo, $slug, $resumo, $conteudo, $categoria, $capa, $publicado, $id]);

            $sucesso = 'Post atualizado com sucesso!';
            $post    = array_merge($post, compact('titulo', 'slug', 'resumo', 'conteudo', 'categoria', 'publicado') + ['imagem' => $capa]);
...
<head>
  ...
  <style>
    .editor-toolbar{display:flex;gap:.3rem;flex-wrap:wrap;align-items:center;padding:.6rem .75rem;background:var(--abg);border:1.5px solid var(--aborda);border-bottom:none;border-radius:8px 8px 0 0;}
    ...
    #editor{min-height:380px;padding:1.25rem;outline:none;border:1.5px solid var(--aborda);border-radius:0 0 8px 8px;font-size:.93rem;line-height:1.8;color:var(--atxt);background:#fff;}
  ...
  <main class="admin-main">
    <div class="topbar">
      ...
      <div style="display:flex;gap:.6rem;">
        <a href="/post/<?= htmlspecialchars($post['slug']) ?>" target="_blank"
           class="btn-admin btn--outline btn--sm">Ver no site ↗</a>
    ...
    <div class="content">
      <?php if ($sucesso): ?>
        <div class="alert alert--sucesso" id="alerta-sucesso">
          ✓ <?= $sucesso ?>
          <a href="/post/<?= htmlspecialchars($post['slug']) ?>" target="_blank"
             style="margin-left:.75rem;font-weight:600;text-decoration:underline;">Ver ↗</a>
        </div>
      <?php endif; ?>
      <?php if ($erro): ?>
      ...
      <form method="POST" id="form-post" enctype="multipart/form-data">
        <input type="hidden" name="remover_capa" id="flag-remover-capa" value="">
          ...
          <div style="display:flex;flex-direction:column;gap:1.25rem;">
            <div class="card" style="padding:1.25rem;">
              ...
              <div class="campo" style="margin-top:1rem;">
                <label>Resumo</label>
                <textarea name="resumo" rows="2"><?= htmlspecialchars($post['resumo']) ?></textarea>
              </div>
            </div>
            ...
            <div class="card" style="padding:1.25rem;">
              ...
              <div class="editor-toolbar">
                <div class="toolbar-grupo">
                  <button type="button" onclick="fmt('bold')"          ><b>N</b></button>
                  <button type="button" onclick="fmt('italic')"        ><i>I</i></button>
                  <button type="button" onclick="fmt('underline')"     ><u>S</u></button>
                  <button type="button" onclick="fmt('strikeThrough')" ><s>T</s></button>
                </div>
                <div class="toolbar-sep"></div>
                <div class="toolbar-grupo">
                  <button type="button" onclick="insTag('h2')"         >H2</button>
                  <button type="button" onclick="insTag('h3')"         >H3</button>
                  <button type="button" onclick="insTag('p')"          >§</button>
                  <button type="button" onclick="insTag('blockquote')" >❝</button>
                </div>
                <div class="toolbar-sep"></div>
                <div class="toolbar-grupo">
                  <button type="button" onclick="fmt('insertUnorderedList')">☰</button>
                  <button type="button" onclick="fmt('insertOrderedList')"  >1.</button>
                </div>
                <div class="toolbar-sep"></div>
                <div class="toolbar-grupo">
              ...
              <div id="editor" contenteditable="true"
                   data-placeholder="Conteúdo do post..."
                   oninput="sincEditor()">
                <?= $post['conteudo'] ?>
              </div>
              <input type="hidden" name="conteudo" id="conteudo-hidden">
              <div style="display:flex;justify-content:space-between;align-items:center;margin-top:.75rem;padding-top:.75rem;border-top:1px solid var(--aborda);">
                <span style="font-size:.72rem;color:var(--atxt2);">
                  Slug: <code style="background:var(--abg);padding:.1rem .4rem;border-radius:4px;"><?= htmlspecialchars($post['slug']) ?></code>
                </span>
                <span style="font-size:.72rem;color:var(--atxt2);">
                  Criado em <?= date('d/m/Y', strtotime($post['publicado_em'])) ?>
                </span>
              </div>
            </div>
          </div>
          ...
          <div style="display:flex;flex-direction:column;gap:1rem;position:sticky;top:80px;">
              ...
              <div class="campo">
                ...
                <select name="categoria">
                  <?php foreach ($cats as $v => $l): ?>
                    <option value="<?= $v ?>" <?= $post['categoria']===$v ? 'selected' : '' ?>><?= $l ?></option>
                  <?php endforeach; ?>
                </select>
              </div>
              <label style="display:flex;align-items:center;gap:.5rem;cursor:pointer;font-size:.85rem;margin-top:.875rem;">
                <input type="checkbox" name="publicado" <?= $post['publicado'] ? 'checked' : '' ?>>
            ...
            <div class="card" style="padding:1.25rem;">
              ...
              <div class="upload-capa" onclick="document.getElementById('input-capa').click()">
                <?php if ($post['imagem']): ?>
                  <img id="capa-preview" src="<?= htmlspecialchars($post['imagem']) ?>"
                       alt="Capa" style="width:100%;height:100%;object-fit:cover;border-radius:8px;">
                  <div class="upload-capa__placeholder" id="capa-placeholder" style="display:none;">
                <?php else: ?>
                  <img id="capa-preview" src="" alt="" style="display:none;width:100%;height:100%;object-fit:cover;border-radius:8px;">
              ...
              <div style="display:flex;gap:.5rem;margin-top:.6rem;">
                <button type="button" onclick="document.getElementById('input-capa').click()"
                        class="btn-admin btn--outline btn--sm" style="flex:1;">
                  <?= $post['imagem'] ? 'Trocar capa' : 'Adicionar capa' ?>
                </button>
                <?php if ($post['imagem']): ?>
                  <button type="button" id="btn-remover-capa" onclick="removerCapa()"
                          class="btn-admin btn--perigo btn--sm">✕</button>
                <?php endif; ?>
              </div>
            </div>
          </div>
...
<div id="modal-img" class="modal-overlay" style="display:none;">
  <div class="modal-box">
    ...
    <div class="campo" style="margin-top:1rem;">
      ...
      <input type="text" id="input-legenda-img" placeholder="Descrição...">
    </div>
...
<!-- MODAL DELETE -->
<div id="modal-delete" style="display:none;position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:999;align-items:center;justify-content:center;">
  <div style="background:#fff;border-radius:14px;padding:2rem;max-width:400px;width:92%;text-align:center;">
    <div style="width:52px;height:52px;background:#FEE2E2;border-radius:50%;display:flex;align-items:center;justify-content:center;margin:0 auto 1rem;">
      <svg viewBox="0 0 24 24" fill="none" stroke="#991B1B" stroke-width="2" style="width:22px;"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6M10 11v6M14 11v6M9 6V4h6v2"/></svg>
    </div>
    <h3 style="font-size:1rem;margin-bottom:.4rem;">Deletar este post?</h3>
    <p style="font-size:.82rem;color:#666;margin-bottom:1.5rem;">Esta ação não pode ser desfeita.</p>
    <div style="display:flex;gap:.75rem;justify-content:center;">
      <button onclick="fecharModal('modal-delete')" class="btn-admin btn--outline">Cancelar</button>
...
function previewCapa(input) {
  ...
  reader.onload = e => {
    ...
    document.getElementById('flag-remover-capa').value = '';
  };
...
function removerCapa() {
  ...
  document.getElementById('flag-remover-capa').value = '1';
  const btn = document.getElementById('btn-remover-capa');
...
['modal-img','modal-video','modal-delete'].forEach(id => {
...
function confirmarDelete(id) {
  document.getElementById('link-delete').href = '/admin/posts.php?deletar=' + id;
  document.getElementById('modal-delete').style.display = 'flex';
}

const alerta = document.getElementById('alerta-sucesso');
if (alerta) setTimeout(() => { alerta.style.transition='opacity .5s'; alerta.style.opacity='0'; }, 4000);
</script>
</body>
</html>